from rabin import encrypt, decrypt, generate_keys, json

def main():
    # size = int(input("Enter size of N: "))
    # generate_keys(size)
    with open("CipherSystems/Rabin/rabin_keys/pub_key.json", "r", encoding="utf-8") as json_file:
        pub_key = json.load(json_file)
    with open("CipherSystems/Rabin/rabin_keys/scrt_key.json", "r", encoding="utf-8") as json_file:
